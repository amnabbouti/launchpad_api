...
class ItemMovementService extends BaseService
{
     ...
     */
    public function getMovementHistory(string|int $itemId, array $filters = []): array
    {
        $data = $this->resolvePublicIds(['item_id' => $itemId]);
        $internalItemId = $data['item_id'];

        $item = Item::findOrFail($internalItemId);
     ...
     */
    public function validateAndRepairItemLocationIntegrity(string|int|null $itemId = null, bool $autoRepair = false): array
    {
        if ($itemId && is_string($itemId)) {
            $resolved = $this->resolvePublicIds(['item_id' => $itemId]);
    ...
    private function validateAndRepairItemLocationIntegrityForItem(?Item $item = null, bool $autoRepair = false): array
    {
        ...
        foreach ($itemsQuery as $currentItem) {
            ...
            $orphanedLocations = ItemLocation::whereNotIn('item_id', function ($query) {
            ...
            if ($orphanedLocations->isNotEmpty()) {
                ...
                if ($autoRepair) {
                    $deleted = ItemLocation::whereNotIn('item_id', function ($query) {
                        $query->select('id')->from('items');
                    })->delete();
     ...
     */
    private function executeInitialPlacement(Item $item, int $toLocationId, float $quantity, $user): array
    {
        if ($quantity <= 0) {
     ...
     */
    private function executeStandardMovement(Item $item, int $fromLocationId, int $toLocationId, float $quantity, $user): array
    {
        if ($quantity <= 0) {
            throw new InvalidArgumentException(__(ErrorMessages::ITEM_MOVEMENT_QUANTITY_POSITIVE));
        }
     ...
     */
    private function createMovementRecord(Item $item, ?int $fromLocationId, int $toLocationId, float $quantity, string $movementType, array $data, $user): ItemMovement
    {
