...
class ItemLocationService extends BaseService {
    ...
    public function findById($id, array $columns = ['*'], array $relations = [], array $appends = []): Model {
        $defaultRelationships = [
            'organization',
            'item' => static function ($query): void {
    ...
    public function getFiltered(array $filters = []): Builder {
        ...
        return $this->getQuery()
            ->with($relationships)
            ->when($filters['location_id'] ?? null, static fn ($q, $id) => $q->where('location_id', $id))
            ->when($filters['item_id'] ?? null, static fn ($q, $id) => $q->where('item_id', $id))
            ->when($filters['moved_date'] ?? null, static function ($q, $date) {
                try {
            ...
            ->when($filters['positive_quantity'] ?? null, static fn ($q) => $q->where('quantity', '>', 0))
    ...
    private function validateBusinessRules(array $data, string $operation = 'general'): void {
        $requiredFieldsMap = [
            'item_location'   => ['item_id', 'location_id', 'quantity'],
            'move'            => ['item_id', 'from_location_id', 'to_location_id', 'quantity'],
            'update_quantity' => ['item_id', 'location_id', 'quantity'],
            'general'         => ['item_id', 'location_id', 'quantity'],
        ];
