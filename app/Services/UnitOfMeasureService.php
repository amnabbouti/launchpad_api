...
class UnitOfMeasureService extends BaseService {
    ...
    public function getFiltered(array $filters = []): Builder {
        return $this->getQuery()
            ...
            ->when($filters['code'] ?? null, static fn ($q, $code) => $q->where('code', $code))
            ->when($filters['type'] ?? null, static fn ($q, $type) => $q->where('type', $type))
     ...
     */
    public function updateUnitOfMeasure(string $id, array $data): Model {
        $data = $this->applyUnitOfMeasureBusinessRules($data);
